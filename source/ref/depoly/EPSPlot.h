...
public:
	...
	void drawText(float x, float y, const char *text, float halign, float valign, float angle) {
		...
		if (!fontsize) setFontSize(6); // Default font size is 6
		gSave();
		printFloatAndSpace(x);
		printFloatAndSpace(y);
		...
		if (halign) {
			...
			printString("mul dup ");
			printFloat(-valign*fontsize);
			printString(" moveto exch\n");
		} else {
			printString(") dup\n");
			moveTo(0, -valign*fontsize);
		}
		printString("true charpath\n");
		setGray(1);
		setNoDash();
		...
		newPath();
		setGray(0);
		if (!halign) printFloatAndSpace(0);
		printFloatAndSpace(-valign*fontsize);
	...
	void drawLegend(float x, float y, char *text) {
		drawLine(x, y, x-15, y);

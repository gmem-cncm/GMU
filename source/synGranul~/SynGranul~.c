...
//void synGranul_setInit(t_synGranul *x, t_symbol *s);	// definition du buffer~ son (à l'initialisation du patch)
void synGranul_swap(t_synGranul *x);					// bascule buffer son
int synGranul_bufferinfos(t_synGranul *x);				// affectation des paramètres des buffers
...
{ 
    ...
    class_addmethod(synGranul_class, (t_method)synGranul_sinterp, gensym("sinterp"), A_FLOAT, 0);	// interpollation dans la lecture du buffer pour éviter les clics
...
{ 
	t_class *c = class_new("synGranul~", (method)synGranul_new, (method)synGranul_free, sizeof(t_synGranul), (method)NULL, A_GIMME, 0L);
    ...
    class_addmethod(c, (method)synGranul_sinterp, "sinterp", A_LONG, 0);	// interpollation dans la lecture du buffer pour éviter les clics
...
void synGranul_perform64(t_synGranul *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam)
{
	...
	else
	{
		...
		// Dans le cas d'un grain déclenché par un bang
		savein = in[0];
   		if(x->x_askfor)
   		{
			sigin = -1.;

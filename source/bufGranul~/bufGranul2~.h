...
// BUFFER SON
	...
	double	 *x_buf_samples[NSBUF];		// pointeur sur le tableau d'�chantillon du buffer~ son
	double     x_buf_sronsr[NSBUF];		// freq ech buffer sur freq ech globale
	int		  x_nbuffer;				// nombre de buffer son
 	int       x_active_buf;				// le buffer son actif est celui dans lequel les nouveaux grains sont pr�lev�s.
// BUFFER SON LOOP
...
// BUFFER ENVELOPPE
	...
	double	 *x_env_samples[NEBUF];		// pointeur sur le tableau d'�chantillon du buffer~ enveloppe
...
//void bufGranul_setInit(t_bufGranul *x, t_symbol *s);	// definition du buffer~ son (� l'initialisation du patch)
void bufGranul_swap(t_bufGranul *x);					// bascule buffer son
int bufGranul_bufferinfos(t_bufGranul *x);				// affectation des param�tres des buffers
